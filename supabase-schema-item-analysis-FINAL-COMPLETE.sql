-- FINAL COMPLETE SQL - Only calculates profit for orders with BOTH commodity AND freight data
-- Strips # from ALL order numbers for consistent matching
...
order_total_costs AS (
    ...
    SELECT
        ...
        -- Total all-in cost (includes commodity + domestic + intl shipping + service fees)
...
order_details AS (
    -- Strip # from Shopify order numbers
    SELECT
...
estimated_item_costs AS (
    SELECT
        ...
        -- COST ALLOCATION - Only for orders with complete data
        CASE
...
SELECT
    eic.*,
    -- Profit calculations - ONLY for orders with complete data
    CASE
...
-- SKU summary view - Only SKUs with complete data
...
WHERE sku IS NOT NULL AND sku != ''
  AND cost_data_source != 'Incomplete Invoice Data'  -- ONLY complete data
  AND has_complete_data = true                        -- ONLY orders in both tables
  AND estimated_unit_cost_usd IS NOT NULL
GROUP BY sku, product_title
ORDER BY total_revenue_usd DESC;


-- Product-level summary view - Only products with complete data
